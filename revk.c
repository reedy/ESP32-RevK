...
mqtt_event_handler (esp_mqtt_event_t * event)
{
  ...
  switch (event->event_id)
	...
	free (topic);
      ...
      revk_info (NULL, "MQTT%d(%d) %s mem=%d %dms L%d", mqtt_index + 1, mqtt_count, p->label, esp_get_free_heap_size (), portTICK_PERIOD_MS, CONFIG_LOG_DEFAULT_LEVEL);
      if (app_command)
...
revk_command (const char *tag, unsigned int len, const unsigned char *value)
{
  ...
  if (!e && !strcmp (tag, "factory") && len == strlen (revk_id) + strlen (revk_app) && !strncmp ((char *) value, revk_id, strlen (revk_id)) && !strcmp ((char *) value + strlen (revk_id), revk_app))
    {
      setting_t *s;
