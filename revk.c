...
revk_task(void *pvParameters)
{
	...
	//Idle
		...
		if (restart_time && restart_time < now) {
			//Restart
				revk_status(NULL, "0 %s", restart_reason ? : "Unknown");
...
nvs_set(setting_t * s, void *data)
{
	...
	if (s->flags & SETTING_BINARY) {
		if (s->size)
			return nvs_set_blob(nvs, s->name, data, s->size);
		//Fixed
			return nvs_set_blob(nvs, s->name, data, 1 + *((unsigned char *)data));
		//Variable
	}
	if (s->size == 0) {
...
revk_register(const char *name, unsigned char array, signed char size, void *data, int64_t defval, unsigned char flags)
{
	...
	if (!s->size) {
		...
		l = nvs_get(s, data, s->size);
	//Stored static
	if              (l < 0) {
		const char     *e = revk_setting(name, 0, NULL);
		              //Defaulting logic
		if              (e && *e)
