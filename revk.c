...
mqtt_event_handler (esp_mqtt_event_t * event)
{
   ...
   case MQTT_EVENT_CONNECTED:
      ...
      revk_info (NULL, "MQTT%d(%d) %s mem=%d %dms L%d", mqtt_index + 1, mqtt_count, p->label, esp_get_free_heap_size (),
                 portTICK_PERIOD_MS, CONFIG_LOG_DEFAULT_LEVEL);
      if (app_command)
...
// Idle
   while (1)
   {
      ...
      {                         // on line
         ...
         {
            revk_info (NULL, "WiFi%d(%d) %02X%02X%02X:%02X%02X%02X %s (%ddB) ch%d%s", wifi_index + 1, wifi_count, ap.bssid[0],
