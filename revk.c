	...
	case MQTT_EVENT_CONNECTED:
		...
		void		sub       (const char *prefix){
			...
			if              (asprintf(&topic, "%s/%s/*/#", prefix, revk_app) < 0)
				...
				sub           (prefixcommand);
		sub(prefixsetting);
			revk_status(NULL, "1 %s", revk_version);
			// Info
...
revk_task(void *pvParameters)
{
	...
	//Idle
		...
		if (restart_time && restart_time < now) {
			...
			sleep(2);
			// Wait for MQTT to close cleanly
					esp_restart();
			restart_time = 0;
		}
...
#undef p
	...
	//Start task
		xTaskCreatePinnedToCore(revk_task, "RevK", 16 * 1024, NULL, 1, &revk_task_id, tskNO_AFFINITY);
	//TODO stack, priority, affinity check ?
}
...
revk_restart(const char *reason, int delay)
{
	...
	//Cancelled
		...
		restart_time = esp_timer_get_time() + 1000000LL * delay;
	//Reboot now
		return "";
	//Done
}
...
ota_handler(esp_http_client_event_t * evt)
{
	...
	case HTTP_EVENT_ON_DATA:
		...
		if (!ota_running && ota_size && esp_http_client_get_status_code(evt->client) / 100 == 2) {
			...
			if (!ota_partition)
				revk_error("upgrade", "No OTA parition available");
			//TODO if running
				in OTA, boot to factory to allow OTA
					else
			{
...
revk_ota(const char *host)
{
	...
	if (ota_task_id)
		return "OTA running";
	xTaskCreatePinnedToCore(ota_task, "OTA", 16 * 1024, (char *)host, 1, &ota_task_id, tskNO_AFFINITY);
	//TODO stack, priority, affinity check ?
		return "";
}
...
nvs_set(setting_t * s, const char *tag, void *data)
{
	...
	if (s->flags & SETTING_BINARY) {
		if (s->size)
			return nvs_set_blob(nvs, tag, data, s->size);
		//Fixed
			return nvs_set_blob(nvs, tag, data, 1 + *((unsigned char *)data));
		//Variable
	}
	if (s->size == 0) {
...
revk_setting(const char *tag, unsigned int len, const unsigned char *value)
{
	...
	int		match      (setting_t * s) {
		...
		if              (*a)
					return	1;
		//not matched whole name, no match
			if (!*b)
			return 0;
		//Match, no index
			if (!s->array && *b)
			return 2;
		//not array, and more characters, no match
			int		v = 0;
		while (isdigit((int)(*b)))
			v = v * 10 + *b++;
		if (*b)
			return 3;
		//More on end after any digits, no match
			if (v > s->array)
			return 4;
		//Too big, no match
			index = v;
...
revk_register(const char *name, unsigned char array, unsigned char size, void *data, const char *defval, unsigned char flags)
{
	...
	if              (l < 0) {
		...
		if              (array) {
			int		i;
			for             (i = 1; i <= array; i++) {
				e = revk_setting_internal(s, 0, NULL, i, SETTING_LIVE);
				//Defaulting logic
				if (e && *e)
			...
			e = revk_setting_internal(s, 0, NULL, 0, SETTING_LIVE);
			//Defaulting logic
				if (e && *e)
				ESP_LOGE(TAG, "Setting %s failed %s", s->name, e);
			else
